...
using Meow.Application.Domain.Core.Repository;
...
namespace Meow.Application.Data.Ef.Core.Store
{
    /// <summary>
    /// 查询存储器
    /// </summary>
    ...
    public abstract class QueryStoreBase<TEntity, TKey> : IQueryStore<TEntity, TKey>
        where TEntity : class, IKey<TKey>
    {
        ...
        {
            var key = Meow.Helper.Common.To<TKey>(id);
            if (key.SafeString().IsEmpty())
                return null;
            return Set.Find(key);
        }
        ...
        {
            var idList = Meow.Helper.String.ToList<TKey>(ids);
