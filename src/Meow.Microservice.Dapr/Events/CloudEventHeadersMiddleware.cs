...
public class CloudEventHeadersMiddleware {
    ...
    protected virtual async Task ImportHeaders( HttpContext httpContext ) {
        ...
        JsonElement jsonElement = await JsonSerializer.DeserializeAsync<JsonElement>( httpContext.Request.Body );
        httpContext.Request.Body.Seek( 0 , SeekOrigin.Begin );
        if( jsonElement.TryGetProperty( "headers" , out JsonElement headers ) == false )
            return;
        Dictionary<string , string> result = headers.Deserialize<Dictionary<string , string>>();
