...
public class DaprIntegrationEventBus : IIntegrationEventBus {
    ...
    private void SetHeaders() {
        IDictionary<string , string> headers = GetImportHeaders();
    ...
    private IDictionary<string , string> GetImportHeaders() {
        ...
        if( ImportHeaderKeys.Count == 0 )
            return result;
        IHeaderDictionary headers = Web.Request?.Headers;
        if( headers == null )
            return result;
    ...
    protected virtual async Task PublishEventAsync( IIntegrationEvent @event , CancellationToken cancellationToken ) {
        ...
        CloudEvent<object> cloudEvent = CreateCloudEvent( @event );
        PubsubArgument argument = CreatePubsubArgument( cloudEvent );
    ...
    protected CloudEvent<object> CreateCloudEvent( IIntegrationEvent @event ) {
        CloudEvent<object> result = new CloudEvent<object>( @event.EventId , @event ) {
            Type = CloudEventType ,
    ...
    public virtual async Task RepublishAsync( string eventId , CancellationToken cancellationToken = default ) {
        ...
        IntegrationEventLog eventLog = await EventManager.GetAsync( eventId , cancellationToken );
    ...
    protected virtual async Task RepublishAsync( IntegrationEventLog eventLog , CancellationToken cancellationToken = default ) {
        ...
        eventLog.SubscriptionLogs.ForEach( t => {
            if( t.State == SubscriptionStateEnum.Fail )
                t.RetryCount = 0;
        } );
        await EventManager.SaveAsync( eventLog , cancellationToken );
        string json = Meow.Helper.Json.ToJson( eventLog.Value );
        PubsubArgument<CloudEvent<object>> argument = Meow.Helper.Json.ToObject<PubsubArgument<CloudEvent<object>>>( json );
        CloudEvent<object> cloudEvent = argument.GetEventData<CloudEvent<object>>();
