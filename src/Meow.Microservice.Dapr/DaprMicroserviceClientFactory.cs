namespace Meow.Microservice.Dapr;
...
public class DaprMicroserviceClientFactory : IMicroserviceClientFactory {
    ...
    public virtual IMicroserviceClient Create() {
        DaprClient client = CreateDaprClient();
    ...
    protected virtual DaprClient CreateDaprClient() {
        string httpEndpoint = GetDaprHttpEndpoint();
        string grpcEndpoint = GetDaprGrpcEndpoint();
    ...
    protected virtual string GetDaprHttpEndpoint() {
        if( _daprHttpPort == 0 )
            _daprHttpPort = Meow.Helper.Environment.GetEnvironmentVariable<int>( "DAPR_HTTP_ENDPOINT" );
    ...
    protected virtual string GetDaprGrpcEndpoint() {
        if( _daprGrpcPort == 0 )
            _daprGrpcPort = Meow.Helper.Environment.GetEnvironmentVariable<int>( "DAPR_GRPC_ENDPOINT" );
    ...
    protected virtual HttpClient CreateHttpClient() {
        ...
        string endpoint = GetDaprHttpEndpoint();
