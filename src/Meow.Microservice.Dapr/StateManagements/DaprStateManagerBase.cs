...
public partial class DaprStateManagerBase<TStateManager> : IStateManagerBase<TStateManager> where TStateManager : IStateManagerBase<TStateManager> {
    ...
    public virtual async Task AddAsync<TValue>( string key , TValue value , CancellationToken cancellationToken = default ) {
        ...
        StateTransactionRequest request = new StateTransactionRequest( key , ToBytes( value ) , StateOperationType.Upsert , null , Metadatas , CreateStateOptions() );
    ...
    protected virtual byte[] ToBytes<TValue>( TValue value ) {
        return Meow.Helper.Json.ToBytes( value , SerializerOptions );
    }
    ...
    public async Task<bool> UpdateAsync<TValue>( string key , TValue value , string etag , CancellationToken cancellationToken = default ) {
        ...
        if( IsTransaction == false ) {
            try {
                bool result = await Client.TrySaveStateAsync( GetStoreName() , key , value , etag , CreateStateOptions() , Metadatas , cancellationToken );
                Clear();
                return result;
            } catch( DaprException ex ) {
                Clear();
                SystemException exception = ex.InnerException;
                if( exception != null && exception.Message.Contains( "E11000" ) )
                    return false;
                throw;
            }
        }
        StateTransactionRequest request = new StateTransactionRequest( key , ToBytes( value ) , StateOperationType.Upsert , etag , Metadatas , CreateStateOptions() );
        StateTransactionRequests.Add( request );
        return true;
    }
    ...
    public async Task RemoveAsync( string key , CancellationToken cancellationToken = default ) {
        ...
        StateTransactionRequest request = new StateTransactionRequest( key , null , StateOperationType.Delete , null , Metadatas , CreateStateOptions() );
        StateTransactionRequests.Add( request );
    }
    ...
    public async Task RemoveByIdAsync<TValue>( string id , CancellationToken cancellationToken = default ) where TValue : IDataKey {
        string key = await GetKeyByIdAsync<TValue>( id , cancellationToken );
    ...
    public async Task<string> SaveAsync<TValue>( TValue value , CancellationToken cancellationToken = default , string key = null ) where TValue : IDataKey, IETag {
        ...
        if( IsTransaction == false ) {
            bool result = await UpdateAsync( key , value , value.ETag ?? string.Empty , cancellationToken );
            if( result == false )
        ...
        StateTransactionRequest request = new StateTransactionRequest( key , bytes , StateOperationType.Upsert , value.ETag ?? string.Empty , Metadatas , CreateStateOptions() );
        StateTransactionRequests.Add( request );
        return key;
    }
    ...
    public async Task SaveAsync<TValue>( IEnumerable<TValue> values , CancellationToken cancellationToken = default ) where TValue : IDataKey, IETag {
        ...
        InitJsonType<TValue>();
        List<SaveStateItem<TValue>> items = ToSaveStateItems( values );
        if( IsTransaction == false ) {
            await Client.SaveBulkStateAsync( GetStoreName() , items , cancellationToken );
            Clear();
            return;
        }
        foreach( SaveStateItem<TValue> item in items ) {
            byte[] bytes = ToBytes( item.Value );
            StateTransactionRequest request = new StateTransactionRequest( item.Key , bytes , StateOperationType.Upsert , item.ETag ?? string.Empty , Metadatas , CreateStateOptions() );
            StateTransactionRequests.Add( request );
        }
    }
    ...
    protected List<SaveStateItem<TValue>> ToSaveStateItems<TValue>( IEnumerable<TValue> values ) where TValue : IDataKey, IETag {
        List<SaveStateItem<TValue>> result = new List<SaveStateItem<TValue>>();
        foreach( TValue value in values ) {
            InitSave( value );
            string key = GetKey( null , value );
            SaveStateItem<TValue> item = new SaveStateItem<TValue>( key , value , value.ETag , CreateStateOptions() , Metadatas );
            result.Add( item );
        }
        return result;
    }
