    ...
    where TQuery : IPage
{
    ...
    protected async Task<IActionResult> SaveAsync(SaveModel request)
    {
        if (request == null)
            return Fail("参数不能为空");
        var creationList = Meow.Helper.Json.ToObject<List<TDto>>(request.CreationList);
        var updateList = Meow.Helper.Json.ToObject<List<TDto>>(request.UpdateList);
        var deleteList = Meow.Helper.Json.ToObject<List<TDto>>(request.DeleteList);
