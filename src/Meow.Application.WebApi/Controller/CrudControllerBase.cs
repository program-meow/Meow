...
public abstract class CrudControllerBase<TDto, TCreateRequest, TUpdateRequest, TQuery> : QueryControllerBase<TDto , TQuery>
    ...
    protected async Task<IActionResult> SaveAsync( SaveModel request ) {
        if( request == null )
            return Fail( "参数不能为空" );
        List<TDto> creationList = Meow.Helper.Json.ToObject<List<TDto>>( request.CreationList );
        List<TDto> updateList = Meow.Helper.Json.ToObject<List<TDto>>( request.UpdateList );
        List<TDto> deleteList = Meow.Helper.Json.ToObject<List<TDto>>( request.DeleteList );
