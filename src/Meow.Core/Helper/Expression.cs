...
public static class Expression {
    ...
    /// </summary>
    /// <param name="expression">表达式,范例：t => t.Name == "A"</param>
    public static Meow.Math.OperatorEnum? GetOperator( SystemExpression expression ) {
        if( expression == null )
            return null;
        switch( expression.NodeType ) {
            case ExpressionType.Lambda:
            ...
            case ExpressionType.Equal:
                return Meow.Math.OperatorEnum.Equal;
            case ExpressionType.NotEqual:
                return Meow.Math.OperatorEnum.NotEqual;
            case ExpressionType.GreaterThan:
                return Meow.Math.OperatorEnum.Greater;
            case ExpressionType.LessThan:
                return Meow.Math.OperatorEnum.Less;
            case ExpressionType.GreaterThanOrEqual:
                return Meow.Math.OperatorEnum.GreaterEqual;
            case ExpressionType.LessThanOrEqual:
                return Meow.Math.OperatorEnum.LessEqual;
            case ExpressionType.Call:
    ...
    private static Meow.Math.OperatorEnum? GetMethodCallExpressionOperator( SystemExpression expression ) {
        ...
        switch( methodCallExpression?.Method?.Name?.ToLower() ) {
            ...
            case "endswith":
                return Meow.Math.OperatorEnum.Ends;
            case "startswith":
                return Meow.Math.OperatorEnum.Starts;
        }
        return null;
    }
    ...
    public static ConstantExpression Constant( object value , SystemExpression expression = null ) {
        SystemType type = GetType( expression );
        if( type == null )
    ...
    public static Expression<Func<T , bool>> ParsePredicate<T>( string propertyName , object value , Meow.Math.OperatorEnum @operator ) {
        ParameterExpression parameter = CreateParameter<T>();
        SystemExpression left = Property( parameter , propertyName );
    ...
    /// </summary>
    /// <typeparam name="T">对象类型</typeparam>
    public static object Value<T>( Expression<Func<T , bool>> expression ) {
    ...
    /// <param name="operator">运算符</param>
    /// <param name="value">值</param>
    public static SystemExpression Operation( SystemExpression left , Meow.Math.OperatorEnum @operator , object value ) {
        switch( @operator ) {
            case Meow.Math.OperatorEnum.Equal:
                return Equal( left , value );
            case Meow.Math.OperatorEnum.NotEqual:
                return NotEqual( left , value );
            case Meow.Math.OperatorEnum.Greater:
                return Greater( left , value );
            case Meow.Math.OperatorEnum.GreaterEqual:
                return GreaterEqual( left , value );
            case Meow.Math.OperatorEnum.Less:
                return Less( left , value );
            case Meow.Math.OperatorEnum.LessEqual:
                return LessEqual( left , value );
            case Meow.Math.OperatorEnum.Starts:
                return StartsWith( left , value );
            case Meow.Math.OperatorEnum.Ends:
                return EndsWith( left , value );
            case Meow.Math.OperatorEnum.Contains:
    ...
    /// <param name="operator">运算符</param>
    /// <param name="value">值</param>
    public static SystemExpression Operation( SystemExpression left , Meow.Math.OperatorEnum @operator , SystemExpression value ) {
        switch( @operator ) {
            case Meow.Math.OperatorEnum.Equal:
                return Equal( left , value );
            case Meow.Math.OperatorEnum.NotEqual:
    ...
    /// <param name="values">参数值列表</param>
    public static SystemExpression Call( SystemExpression instance , string methodName , SystemType[] paramTypes , params object[] values ) {
        if( instance == null )
            throw new ArgumentNullException( nameof( instance ) );
    ...
    internal static Expression<T> Compose<T>(
        ...
        SystemExpression secondBody = ParameterRebinder.ReplaceParameters( map , second.Body );
    ...
    public static Expression<TDelegate> ToLambda<TDelegate>( SystemExpression body , params ParameterExpression[] parameters ) {
        if( body == null )
            return null;
    ...
    /// <param name="body">表达式</param>
    /// <param name="parameters">参数列表</param>
    public static Expression<Func<T , bool>> ToPredicate<T>( SystemExpression body , params ParameterExpression[] parameters ) {
