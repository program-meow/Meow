...
public class Bootstrapper
{
    ...
    /// 服务配置操作列表
    /// </summary>
    private readonly List<System.Action> _serviceActions;
    ...
    public Bootstrapper(IHostBuilder hostBuilder, Action<Options> setupAction = null)
    {
        ...
        _serviceActions = new List<System.Action>();
    }
    ...
    protected virtual void ResolveServiceRegistrar()
    {
        ...
        var instances = types.Select(type => Helper.Reflection.CreateInstance<IServiceRegistrar>(type)).Where(t => t.Enabled).OrderBy(t => t.OrderId).ToList();
    ...
    protected virtual void ResolveDependencyRegistrar()
    {
        ...
        var instances = types.Select(type => Helper.Reflection.CreateInstance<IDependencyRegistrar>(type)).OrderBy(t => t.Order).ToList();
        _hostBuilder.ConfigureServices((context, services) =>
        {
