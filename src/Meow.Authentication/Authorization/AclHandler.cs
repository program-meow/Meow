...
public class AclHandler : AuthorizationHandler<AclRequirement> {
    ...
    protected override async Task HandleRequirementAsync( AuthorizationHandlerContext context , AclRequirement requirement ) {
        ...
        if( requirement == null )
            return;
        HttpContext httpContext = GetHttpContext( context.Resource );
        if( httpContext == null )
            return;
        ...
        IPermissionManager permissionManager = httpContext.RequestServices.GetService<IPermissionManager>();
        if( permissionManager == null ) {
            context.Fail( new AuthorizationFailureReason( this , "未实现IPermissionManager" ) );
            return;
        }
        string uri = GetResourceUri( requirement , httpContext );
        bool result = await permissionManager.HasPermissionAsync( uri );
        if( result ) {
            context.Succeed( requirement );
            return;
        }
