...
/// Join子句操作扩展
/// </summary>
public static class JoinClauseExtension
{
    #region Join  [内连接]
    ...
    public static T On<T>(this T source, string column, object value, OperatorEnum @operator = OperatorEnum.Equal, bool isParameterization = false) where T : IJoin
    {
        source.CheckNull(nameof(source));
        if (source is ISqlPartAccessor accessor)
