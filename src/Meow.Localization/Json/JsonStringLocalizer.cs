...
public class JsonStringLocalizer : StringLocalizerBase {
    ...
    protected override LocalizedString GetLocalizedString( CultureInfo culture , string name ) {
        string value = GetValue( culture , name , _resourcesBaseName ) ?? GetValue( culture , name , null );
        if( value.IsEmpty() )
    ...
    protected override string GetValue( CultureInfo culture , string name , string type ) {
        IEnumerable<KeyValuePair<string , string>> resources = GetResourcesByCache( culture , type );
        if( resources == null )
            return null;
        KeyValuePair<string , string> resource = resources.SingleOrDefault( t => t.Key == name );
        string result = resource.Value;
    ...
    protected virtual IEnumerable<KeyValuePair<string , string>> GetResources( CultureInfo culture , string resourcesBaseName ) {
        string path = _pathResolver.GetJsonResourcePath( _resourcesRootPath , resourcesBaseName , culture );
    ...
    public override IEnumerable<LocalizedString> GetAllStrings( bool includeParentCultures ) {
        ...
        foreach( CultureInfo culture in cultures ) {
            IEnumerable<LocalizedString> resources = ToLocalizedStrings( GetResourcesByCache( culture , _resourcesBaseName ) );
