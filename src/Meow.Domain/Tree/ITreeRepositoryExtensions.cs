...
public static class ITreeRepositoryExtensions
{
    ...
    {
        UpdatePathManager<TEntity, TKey, TParentId> manager = new UpdatePathManager<TEntity, TKey, TParentId>(repository);
    ...
    {
        return entities.GetMissingParentIds<TEntity, Guid, Guid?>();
    }
    ...
    public static List<string> GetMissingParentIds<TEntity, TKey, TParentId>(this IEnumerable<TEntity> entities) where TEntity : class, ITreeEntity<TEntity, TKey, TParentId>
    {
        ...
        IEnumerable<string> ids = list.Select(t => t?.Id.SafeString());
