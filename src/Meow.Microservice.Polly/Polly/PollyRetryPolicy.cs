namespace Meow.Microservice.Polly;
...
public class PollyRetryPolicy : IRetryPolicy {
    ...
    /// 重试回调函数
    /// </summary>
    private Action<SystemException , int> _onRetry;
    ...
    /// <inheritdoc />
    public IRetryPolicy OnRetry( Action<SystemException , int> action ) {
        _onRetry = action;
        return this;
    }
    ...
    /// <inheritdoc />
    public void Execute( SystemAction action ) {
    ...
    /// 重试处理函数
    /// </summary>
    private void OnRetry( SystemException exception , int times ) {
        if( _onRetry == null )
            return;
    ...
    private Func<int , TimeSpan> GetSleepDurationProvider() {
        if( _sleepDurationProvider == null )
            return retryAttempt => TimeSpan.FromSeconds( SystemMath.Pow( 2 , retryAttempt ) );
