...
public class DomainTenantResolver : TenantResolverBase {
    ...
    protected override async Task<string> Resolve( HttpContext context ) {
        string host = DomainTenantResolverHelper.RemoveDomainPrefix( context.Request.Host.Value );
        if( host.IsEmpty() )
            return null;
        string result = await Resolve( context , host );
    ...
    private async Task<string> Resolve( HttpContext context , string host ) {
        ITenantDomainStore store = context.RequestServices.GetService<ITenantDomainStore>();
        IDictionary<string , string> map = await DomainTenantResolverHelper.CombineMapAsync( DomainMap , store );
