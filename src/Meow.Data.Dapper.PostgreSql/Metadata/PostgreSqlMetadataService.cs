...
public class PostgreSqlMetadataService : IMetadataService
{
    ...
    {
        DatabaseInfo result = await GetDatabase();
        List<TableInfo> tables = await GetTables();
    ...
    private async Task<List<TableInfo>> GetTables()
    {
        ...
        List<TableInfo> tables = (await _sqlQuery.ToListAsync<TableInfo, ColumnInfo, TableInfo>((table, column) =>
        {
