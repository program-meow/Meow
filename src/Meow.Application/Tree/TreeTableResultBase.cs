...
public abstract class TreeTableResultBase<TSourceNode, TDestinationNode, TResult>
    ...
    public TResult GetResult() {
        ...
        foreach( TSourceNode root in GetRootNodes() )
            AddNode( root );
    ...
    protected virtual void AddNode( TSourceNode node ) {
        if( node == null )
            return;
        InitNode( node );
        TDestinationNode destinationNode = ToDestinationNode( node );
        AddDestinationNode( destinationNode );
        List<TSourceNode> children = GetChildren( node );
        foreach( TSourceNode child in children )
            AddNode( child );
    }
    ...
    protected virtual bool IsLeaf( TSourceNode node ) {
        if( _data.All( t => t.ParentId != node.Id ) )
            return true;
        List<TSourceNode> children = _data.FindAll( t => t.ParentId == node.Id );
