...
namespace Meow.Tests.Helpers
{
    ...
    public class StringTest
    {
        ...
        {
            Assert.Equal("1,2,3", Meow.Helpers.String.Join(new List<int> { 1, 2, 3 }));
            Assert.Equal("'1','2','3'", Meow.Helpers.String.Join(new List<int> { 1, 2, 3 }, "'"));
            Assert.Equal("123", Meow.Helpers.String.Join(new List<int> { 1, 2, 3 }, "", ""));
            Assert.Equal("\"1\",\"2\",\"3\"", Meow.Helpers.String.Join(new List<int> { 1, 2, 3 }, "\""));
            Assert.Equal("1 2 3", Meow.Helpers.String.Join(new List<int> { 1, 2, 3 }, "", " "));
            Assert.Equal("1;2;3", Meow.Helpers.String.Join(new List<int> { 1, 2, 3 }, "", ";"));
            Assert.Equal("1,2,3", Meow.Helpers.String.Join(new List<string> { "1", "2", "3" }));
            Assert.Equal("'1','2','3'", Meow.Helpers.String.Join(new List<string> { "1", "2", "3" }, "'"));
